...
(function ($) {
  ...
  handler = function handler($selector, event, childSelector, fn, skipNS) {
  ...
  getTabIndex = function getTabIndex(content, url) {
  ...
  trimText = function trimText(text, maxLength) {
  ...
  supportStorage = function supportStorage(is_cache) {
  ...
  sumDomWidth = function sumDomWidth(JqueryDomObjList) {
    var width = 0;
  ...
  isExtUrl = function isExtUrl(url) {
  ...
  MultiTabs = function MultiTabs(element, options) {
    var self = this;
  ...
  MultiTabs.prototype = {
     ...
     */
    create: function create(obj, active) {
      ...
      param.active = !param.active ? active : param.active; //nav tab create

      $navTab = this._createNavTab(param); //tab-pane create

      this._createTabPane(param); //add tab to storage
     ...
     * @private
     */
    _createTabPane: function _createTabPane(param) {
      var self = this,
          $el = self.$element;
     ...
     * @private
     */
    _getTabPaneHtml: function _getTabPaneHtml(param) {
      var self = this,
     ...
     * @private
     */
    _createNavTab: function _createNavTab(param) {
      var self = this,
          $el = self.$element;
     ...
     * @private
     */
    _getNavTabHtml: function _getNavTabHtml(param) {
      var self = this,
          options = self.options;
     ...
     * @private
     */
    _generateId: function _generateId(param) {
     ...
     */
    active: function active(navTab, isNavBar) {
      var self = this,
          $el = self.$element;
      ...
      var navTabParam = $navTab.length ? self._getParam($navTab) : {}; //change storage active status
      ...
      self._resetStorage(storage); //active navTab and tabPane
      ...
      self._fixTabContentLayout($tabPane); //fill tab pane
     ...
     * @private
     */
    _fillTabPane: function _fillTabPane(tabPane, param, isNavBar) {
      var self = this,
          options = self.options;
      var $tabPane = $(tabPane); //if navTab-pane empty, load content
    ...
    close: function close(navTab) {
      ...
      $tabPane = self._getTabPane($navTab); //close unsave tab confirm
      ...
      if ($navTabLi.find('a').hasClass("active")) {
        var $nextLi = $navTabLi.next("li:first"),
            $prevLi = $navTabLi.prev("li:last"); //if ($nextLi.size()) {

        if ($nextLi.length) {
          self.active($nextLi);
          self.activeMenu($nextLi.find('a')); //} else if ($prevLi.size()) {
     ...
     * @return self     Chain structure.
     */
    closeOthers: function closeOthers(retainTab) {
      var self = this,
          $el = self.$element,
          findTab;
     ...
     * 左侧导航变化
     */
    activeMenu: function activeMenu(navTab) {
      // 点击选项卡时，左侧菜单栏跟随变化
      var $navObj = $("a[href$='" + $(navTab).data('url') + "']"),
          // 当前url对应的左侧导航对象
      ...
      $navHasSubnav.addClass('active'); // 当前菜单无子菜单
    ...
    _init: function _init(options) {
      ...
      $el.navPanelList = $el.nav.find('.mt-nav-panel:first ul'); //$el.navTabMain    = $('#multitabs_main_0');
    ...
    _listen: function _listen() {
      var self = this,
          $el = self.$element,
          options = self.options; //create tab
      ...
      if (options.nav.draggable) {
        handler($el.navPanelList, 'mousedown', '.mt-nav-tab', function (event) {
          ...
          $(document).on('mousemove', function (event) {
            ...
            if (dragMode) {
              $navTabLi.removeClass('mt-dragging mt-dragging-tab').css({
                'left': 'auto'
              });
      ...
      handler($el.nav, 'click', '.mt-show-actived-tab', function () {
        self.showActive(); //return false; //Avoid possible BUG
      }); //close all tabs

      handler($el.nav, 'click', '.mt-close-all-tabs', function () {
        self.closeAll(); //return false; //Avoid possible BUG
      }); //close other tabs

      handler($el.nav, 'click', '.mt-close-other-tabs', function () {
        self.closeOthers(); //return false; //Avoid possible BUG
      }); //fixed the nav-bar
     ...
     * @private
     */
    _getParam: function _getParam(obj) {
      ...
      param.isNewTab = data.hasOwnProperty('isNewTab') || obj.hasOwnProperty('isNewTab') || options.isNewTab; //iframe

      param.iframe = data.iframe || obj.iframe || isExtUrl(param.url) || options.iframe; //type

      param.type = data.type || obj.type || options.type; //title

      param.title = data.title || obj.title || $obj.text() || param.url.replace('http://', '').replace('https://', '') || options.language.nav.title;
      param.title = trimText(param.title, options.nav.maxTitleLength); //active

      param.active = data.active || obj.active || false; //index

      param.index = data.index || obj.index || getTabIndex(param.type, param.url); //id
     ...
     * @private
     */
    _storage: function _storage(key, param) {
     ...
     * @private
     */
    _delStorage: function _delStorage(key) {
      if (supportStorage(this.options.cache)) {
        var storage = JSON.parse(sessionStorage.multitabs || '{}');
     ...
     * @private
     */
    _resetStorage: function _resetStorage(storage) {
      if (supportStorage(this.options.cache) && _typeof(storage) === "object") {
        sessionStorage.multitabs = JSON.stringify(storage);
      }
    },
     ...
     * @private
     */
    _getTabPane: function _getTabPane(navTab) {
     ...
     * @private
     */
    _getNavTab: function _getNavTab(navTab) {
      var self = this,
          $el = self.$element;
     ...
     * @private
     */
    _fixTabPosition: function _fixTabPosition(navTab) {
      var self = this,
          $el = self.$element,
          $navTabLi = $(navTab).parent('li'),
          ...
          navPanelWidth = $el.navPanel.outerWidth(true),
          sumTabsWidth = sumDomWidth($el.navPanelList.children('li')),
          leftWidth = 0; //all nav navTab's width no more than nav-panel's width
    ...
    _getHiddenList: function _getHiddenList() {
      var self = this,
          ...
          tabNextList = [],
          $navTabLi,
          marginLeft; //all tab's width no more than nav-panel's width

      if (sumTabsWidth < navPanelWidth) {
        return false;
      } else {
        $navTabLi = $el.navPanelList.children('li:first'); //overflow hidden left part

        marginLeft = 0; //from the first tab, add all left part hidden tabs
     ...
     * @private
     */
    _fixTabContentLayout: function _fixTabContentLayout(tabPane) {
      var $tabPane = $(tabPane);
  ...
  $.fn.multitabs.defaults = {
    selector: '.multitabs',
    // selector text to trigger multitabs.
    iframe: false,
    // Global iframe mode, default is false, is the auto mode (for the self page, use ajax, and the external, use iframe)
    cache: false,
    // 是否缓存当前打开的tab
    "class": '',
    // class for whole multitabs
    type: 'info',
    // change the info content name, is not necessary to change.
    init: [],
    isNewTab: false,
    // 是否以新tab标签打开，为true时，每次点击都会打开新的tab
    refresh: 'no',
    // iframe中页面是否刷新，'no'：'从不刷新'，'nav'：'点击菜单刷新'，'all'：'菜单和tab点击都刷新'
    dbclickRefresh: false,
    // 双击刷新开启最好不要和refresh:'all'同时使用
    nav: {
      ...
      //class of nav
      draggable: true,
      //nav tab draggable option
      fixed: false,
      //fixed the nav-bar
      layout: 'default',
      //it can be 'default', 'classic' (all hidden tab in dropdown list), and simple
      showTabs: true,
      // 是否显示Tab标签栏
      maxTitleLength: 25,
      //Max title length of tab
      showCloseOnHover: false,
      //while is true, show close button in hover, if false, show close button always
    ...
    content: {
      ajax: {
        "class": '',
        //Class for ajax tab-pane
        error: function error(htmlCallBack) {
          //modify html and return
          return htmlCallBack;
        },
        success: function success(htmlCallBack) {
          //modify html and return
          return htmlCallBack;
        }
      },
    ...
    language: {
      ...
      nav: {
        title: 'Tab',
        //default tab's tittle
        dropdown: '<i class="mdi mdi-menu"></i>',
        //right tools dropdown name
